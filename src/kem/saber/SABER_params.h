 ...
 *
 * In the original SABER code, this was done by uncommenting one of the
 * lines below. In liboqs we instead pass SABER_L on the compiler
 * command line for each variant (2, 3, 4), so we only define a default
 * if SABER_L is not already provided.
 */
...
#ifndef SABER_L
    #define SABER_L 3 /* Saber (default) */
#endif
