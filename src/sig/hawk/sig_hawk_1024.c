...
OQS_SIG *OQS_SIG_hawk_1024_new(void) {
    ...
    sig->claimed_nist_level = 5;   // Hawk-1024 is NIST-V
    sig->euf_cma = true;
    sig->suf_cma = false;
    ...
    sig->keypair = OQS_SIG_hawk_1024_keypair;
    sig->sign = OQS_SIG_hawk_1024_sign;
    sig->verify = OQS_SIG_hawk_1024_verify;
    sig->sign_with_ctx_str = OQS_SIG_hawk_1024_sign_with_ctx_str;
    sig->verify_with_ctx_str = OQS_SIG_hawk_1024_verify_with_ctx_str;

    return sig;
}

/* Low-level HAWK-1024 functions from hawk1024/api.c,
 * after name-mangling in CMake with:
 *   -Dcrypto_sign_keypair=hawk1024_crypto_sign_keypair
 *   -Dcrypto_sign=hawk1024_crypto_sign
 *   -Dcrypto_sign_open=hawk1024_crypto_sign_open
 */
extern int hawk1024_crypto_sign_keypair(unsigned char *pk, unsigned char *sk);
extern int hawk1024_crypto_sign(unsigned char *sm, unsigned long long *smlen,
                                const unsigned char *m, unsigned long long mlen,
                                const unsigned char *sk);
extern int hawk1024_crypto_sign_open(unsigned char *m, unsigned long long *mlen,
                                     const unsigned char *sm, unsigned long long smlen,
                                     const unsigned char *pk);

OQS_API OQS_STATUS OQS_SIG_hawk_1024_keypair(uint8_t *public_key, uint8_t *secret_key) {
    int rc = hawk1024_crypto_sign_keypair(public_key, secret_key);
...
OQS_API OQS_STATUS OQS_SIG_hawk_1024_sign(uint8_t *signature, size_t *signature_len,
    ...
    unsigned long long sm_cap =
        (unsigned long long) message_len + (unsigned long long) OQS_SIG_hawk_1024_length_signature;
    ...
    int rc = hawk1024_crypto_sign(sm, &out_smlen,
    ...
    if (sig_len > OQS_SIG_hawk_1024_length_signature) {
        OQS_MEM_secure_free(sm, sm_cap);
        return OQS_ERROR;
    }
...
OQS_API OQS_STATUS OQS_SIG_hawk_1024_verify(const uint8_t *message, size_t message_len,
    ...
    unsigned long long smlen =
        (unsigned long long) message_len + (unsigned long long) signature_len;
    ...
    int rc = hawk1024_crypto_sign_open(m_out, &m_out_len, sm, smlen, public_key);

    OQS_MEM_secure_free(sm, smlen);
    OQS_MEM_secure_free(m_out, message_len);

    return (rc == 0 && m_out_len == (unsigned long long) message_len)
               ? OQS_SUCCESS
               : OQS_ERROR;
}
...
#endif // OQS_ENABLE_SIG_hawk_1024
